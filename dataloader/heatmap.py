...
# sys.path.append('./src')
from .utils import calc_aabb, cut_image,  flip_image, convert_image_by_pixformat_normalize, reflect_pose, reflect_lsp_kp
...
def heat_2(kps2d):
    ...
    for i in range(0,17):
        x = kps2d[i][0]
        y = kps2d[i][1]
        heatmap = gaussian(x, y, 56, 56)
        # heatmap = gaussian(x, y, 1000, 1000)
...
class hum36m_dataloader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        kps[:, :1] *= ratio_x
        kps[:, 1:2] *= ratio_y
        # ratio = 1.0 * 1.0 / image.shape[0] #args.crop_size = 224 for resnet
        # kps[:, :2] *= ratio
        # kp_3d[:, :3] *= ratio
        # dst_image = cv2.resize(image, (128,128), interpolation = cv2.INTER_CUBIC) for unet
        # if image.shape == None:
        #     print(image_path)
        dst_image = cv2.resize(image, (256,256), interpolation = cv2.INTER_CUBIC) # for stacked
...
if __name__ == '__main__':
    ...
    h36m = hum36m_dataloader(root_data_path, True, [1.1, 2.0], False, 5, flip_prob = 1)
    l = len(h36m)
